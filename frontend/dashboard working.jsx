...
class Dashboard extends Component{
  ...
  checkSimilarity(score, chatbot){
    this.setState({
      chatbots[chatbot]["score"]: score,
      chatbots[chatbot]["similarity"]: true,
    })
  }
  ...
  comparison(){
    for (var key in this.state.chatbots) {
      if (this.state.chatbots.hasOwnProperty(key)) {
        if (this.state.chatbots.key.check) {
          let req = { responses: [this.state.chatbots.key.response, this.state.jamie.response]}
          try {
            this.APICallResponseCompare(req, this.checkSimilarity)
          } catch(e) {
  ...
  async handleClick(){
    ...
    for (var key in this.state.chatbots) {
      if (this.state.chatbots.hasOwnProperty(key)) {
        this.setState({chatbots.key.similarity: false})
      }
    }
    ...
    var promiseArray = []
    // make askJamie call
    this.setState({jamie.loading: true})
    ...
    for (var key in this.state.chatbots) {
      if (this.state.chatbots.hasOwnProperty(key)) {
        if (this.state.chatbots.key.check) {
          this.setState({chatbots.key.loading: true})
          var promise = this.state.chatbots.key.apiCall(params)
          promiseArray.push(promise)

          promise.then( res => {
            let summarized = this.summarizer(res)
            this.setState({
              chatbots.key.response: summarized,
              chatbots.key.loading: false,
            })
          })
        }
      }
  ...
  for (var key in this.state.chatbots) {
    if (this.state.chatbots.hasOwnProperty(key)) {
      this.setState({
        chatbots.key.response: "",
        chatbots.key.similarity: false
      })
    }
  }
